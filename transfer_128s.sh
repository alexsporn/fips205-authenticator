...
# Transfer IOTA using an SLH-DSA-SHA2-128s abstract account.

# === Configuration ===
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
KEY_DIR="$SCRIPT_DIR/keys/128s"
...
if [ ! -f "$ADDRESS_FILE" ]; then
    echo "ERROR: No 128s account found. Run ./create_account.sh 128s first."
    exit 1
fi
...
# === Sign the TX digest ===
echo ""
echo "Signing with SLH-DSA-SHA2-128s..."
SIGNATURE_HEX=$("$CLI" sign \
    --digest "$TX_DIGEST_HEX" \
    --secret-key "$SECRET_KEY" \
    --address "$ABSTRACT_ACCOUNT" \
    --param 128s)
echo "Signature: $((${#SIGNATURE_HEX} / 2)) bytes"
